...
## 6. Estado 2026-07-03 · PENDIENTE

**Hecho el 2026-07-02:** Fase 1 ✅ · Fase 2 ✅ · Fase 3 (seguridad ✅ + Retrofit motor ✅) · sueltos MEDIO ✅ · deps/Dependabot/OWASP ✅. Todo en `origin/main`, 136 tests verdes, verificado en emulador.

**Hecho el 2026-07-03:**
- ✅ **CI — GitHub Actions** (`.github/workflows/ci.yml`, job `api-build-test`): `mvnw verify` (JDK 21) en cada push a `main`/PR con MariaDB 11 efímera (service container), perfil `ci`. `mvnw` invocado con `sh` (fichero en modo 644). Tres bloqueos resueltos en el proceso: (a) **fuentes jOOQ VERSIONADAS** en `src/generated/jooq` (antes en `target/`, gitignored → clone/CI no compilaba; `skip=true` se mantiene, regenerar sobrescribe); (b) **perfil `ci`** (`application-ci.properties`, versionado, datasource a la BD efímera + secreto JWT no productivo, porque `application-dev/prod.properties` NO se versionan); (c) **`LOG_DIR` de logback overridable** (`${LOG_DIR:-/home/ubuntu/logs}`) redirigido a `runner.temp` (la ruta del server AWS no existe en el runner); (d) **drift esquema↔entidad**: la migración inicial creó `alimentos.proteinas` como INT pero la entidad la mapea `DECIMAL(5,2)` → `validate` tumbaba el contexto en la BD construida desde cero por Flyway (el 3308 local no lo detectaba por drift manual); fix en migración `V202607030700`. Verificado con BD nueva vacía + Flyway desde cero + validate: 136 tests, BUILD SUCCESS.
- ✅ **Gate de Dependabot:** Ruleset en `main` ACTIVO — *Require a pull request* (0 approvals) + *Require status checks* = `api-build-test` + bypass del rol Repository-admin (para seguir pusheando directo). Ahora el auto-merge de Dependabot no fusiona hasta que el CI pase.

**Pendiente (orden sugerido):**
1. **Retrofit etapa 2 (tipada)** — Gson + modelos, eliminar `UtilJSONParser`, migrar las 31 Activities a `Callback<T>`. Bloque grande y arriesgado; verificar pantalla a pantalla en emulador.
2. **Fase 4 resto:** Actuator (`/actuator/health`) + tests de nutrición/mediciones + `minifyEnabled true` + despliegue PaaS (Aiven MySQL + Koyeb/Render) + HTTPS.
