...
- **Ninguna ruta nueva con id de recurso sin su test de acceso ajeno** (DEC-014). Los tests de controlador con el servicio simulado no valen: hay que levantar el contexto (`AbstractOwnershipTest`). Qué tiene que comprobar ese test depende de **quién es el dueño del id** (DEC-027): si es un recurso con dueño —una sesión, una comida, un usuario—, **403**; si es un id del catálogo público —un ejercicio, un alimento—, no hay id ajeno que rechazar y lo que se comprueba es el **aislamiento**: lista vacía o `count: 0`. Los contadores y los `exists` cuentan como datos ajenos y se protegen igual.
  **Y el id no tiene por qué venir en la ruta: cuenta igual el que llega en el cuerpo de una escritura.** Un `POST` o un `PUT` que devuelve el recurso referenciado es también una lectura, y necesita su comprobación de propiedad. El barrido de GP-002 a GP-005 miró rutas `GET` y por eso se escapó que `POST /alimentos-comida` aceptaba el `alimentoId` de otro usuario y devolvía su nombre y sus macros. Un alimento del catálogo sí se acepta: manda el dueño del id, no el verbo.
...
- **Ningún fallo de seguridad descrito en el repositorio antes de que su arreglo esté en producción.** El repositorio es público. Hasta que el propietario confirme que el arreglo está desplegado, ni `documentacion/estado`, ni el mensaje del commit, ni el `CHANGELOG.md` explican cómo se explotaba: dicen qué se endurece, y el detalle va solo en el resumen al propietario. Lo ya publicado antes de esta regla se queda como está: reescribir `main` no borra un historial que ya está fuera y rompería la rama de la que despliega Render.
